...
async function chat(msg: string) {
    const history: Content[] = await convo.getHistory();
    const msgObj = { role: "user", parts: [{ text: msg }] };
    const contents: Content[] = [...history, msgObj as Content];

    const { totalTokens } = await model.countTokens({ contents });
    console.log(totalTokens, "tokens in the prompt");

    if (totalTokens <= 2000) {
        const reply = await convo.sendMessage(msg);
        console.log(reply.response.text());
    } else {
        console.log("too long prompt message");
    }
}

chat("I have problem with my deep sleep amount. It's low and high randomly.");
